...
func get_chain_targets(_targets: Array):
	if(_targets == null || _targets.size() == 0 || _targets.size() >= chains || _targets.back() == null):
		return _targets
	...
	#(_targets.back() as Node2D).interse
...
func update_target_line() -> void:
	if(target_line != null):
		if(chain_targets.size() > 0 && chain_targets[0] != null):
			...
			var line_points := [global_position + get_center_offset()]
			for target in chain_targets:
			...
			target_line.visible = true
			target_line.set_points([global_position + get_center_offset(), target.global_position])
		else:
